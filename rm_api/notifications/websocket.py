...
def _on_message(api: "API", message: str):
    ...
    if message_event == "SyncComplete":
        ...
        print(
            f"{Fore.YELLOW}Warning unknown notification event: {message_event}{Fore.RESET}"
        )
...
def _listen(api: "API"):
    ...
    ws = websocket.WebSocketApp(
        ws_url,
        on_message=lambda _, msg: on_message(api, msg),
        header={"Authorization": f"Bearer {api.token}"},
    )
    ws.run_forever(ping_interval=10, reconnect=5)
