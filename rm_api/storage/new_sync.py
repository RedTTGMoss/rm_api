...
def handle_new_api_steps(api: "API"):
    token_info = jwt.decode(
        api.token, algorithms=["HS256"], options={"verify_signature": False}
    )
    ...
    api.document_notifications_uri = api.document_storage_uri = (
        f"https://{tectonic}.tectonic.remarkable.com/"
    )
